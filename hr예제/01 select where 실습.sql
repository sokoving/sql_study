...
-- *보다는 컬럼명을 전부 명시해 주는 게 관리나 가독성 면에서 좋다
...
-- 실습 2
-- 1. employees테이블에서 job_id가 'IT_PROG'인 사원의
-- first_name, job_id, department_id를 조회하세요.
SELECT
...
-- 2. employees테이블에서 last_name이 'King'인 사원의
-- first_name, last_name, hire_date를 조회하세요.
SELECT
...
-- 3. employees테이블에서 salary가 15000보다 크거나 같은 사원의
-- first_name, salary, hire_date를 조회하세요.
SELECT
...
-- 4. employees테이블에서 salary가 10000과 12000 사이에 있는 사원의
-- first_name, salary를 조회하세요.
SELECT
    first_name, salary
FROM employees
...
-- 5. employees테이블에서 manager_id가 101, 102, 103중에 속하는 사원의
-- first_name, salary, manager_id를 조회하세요.
SELECT
...
-- 6. employees테이블에서 job_id가 'IT_PROG', 'FI_MGR'중에 포함되지 않는 사원의
-- first_name, job_id를 조회하세요.
SELECT
    first_name, job_id
FROM employees
...
-- 7. employees테이블에서 job_id가 'IT'로 시작하는 사원의
-- first_name, last_name, job_id를 조회하세요.
SELECT
...
-- 8. employees테이블에서 email의 두번째 글자가 A인 사원의
-- first_name, email을 조회하세요.
SELECT
    first_name, email
FROM employees
WHERE email LIKE '_A%'
...
-- 9. employees테이블에서 manager_id가 null인 사원의
-- first_name, manager_id를 조회하세요.
SELECT
...
-- 10. employees테이블에서 commission_pct가 null이 아닌 사원의
-- first_name, job_id, commission_pct를 조회하세요.
SELECT
